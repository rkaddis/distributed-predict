...
def video_inference(video : cv2.VideoCapture | str, model : str = None, device : int | str = "cpu", classes : list = None) -> dict:
    '''
    ...
    while(video.isOpened()):
        ...
        for i in range(len(result.names)):
            results[f"frame_{frame_id}_{i}"] = {"class":result.names[i], "conf":result.probs[i], "box":result.boxes[i]}
